...
_Non-negotiable rules for how AI agents operate with Bryan Kemp. Twenty-one numbered rules covering verification, communication, scope fidelity, and operational hygiene. Read at the start of every session and apply throughout._
...
_Generic session context for Bryan Kemp. Personal background, communication defaults, anti-Microsoft constraint. Use when no project-specific anchor applies. Pair with the `charter` rule._
...
_Project context for the Tek Design System (bbkemp/tek-design-system). Pipeline, packages, stakeholders, Bryan-specific overlays. Use when working on design-system code, tokens, components, or pipeline. Pair with the `charter` rule._
...
_Project context for Wii U / DS / retro-gaming homebrew work. Hardware state, references, working style. Use when troubleshooting Aroma, RetroArch, UWUVCI, or related setups. Pair with the `charter` rule._
